...
Mobile.setText(findTestObject('Payment/textbox_amountpay'), '50', 0)

Mobile.tap(findTestObject('Payment/btn_makepayment'), 0)

double newAmount = amountDouble - 50.00
...
if (amountbri.find()) {
    ...
    if (extractedAmountBri == GlobalVariable.Global_amountbri) {
        ...
        if (name.toLowerCase().contains('payment') && amountdetail.toLowerCase().contains('$50.00')) {
